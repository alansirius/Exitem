...
const ZOTERO_GPT_EMBEDDING_MODEL = "text-embedding-ada-002";
...
export async function extractLiteratureReview(
  ...
  if (!hasConfiguredAPI) {
    report(42, "检查 AI 配置");
    throw new ReviewUserError(
      "Zotero GPT config missing",
      "未读取到 Zotero GPT 配置，请先在 Zotero GPT 插件中设置 api / secretKey / model / temperature / embeddingBatchNum",
    );
  }
...
async function extractByCompatibleGPTPlugin(
  ...
  if (!detection.installed) {
    throw new ReviewUserError(
      ...
      "未检测到可兼容的 GPT 插件（如 Zotero GPT / Awesome GPT），请切换为 Zotero GPT 配置模式",
    );
  }
  if (detection.installed && !detection.callable) {
    throw new ReviewUserError(
      ...
      detection.obstacle ||
        "检测到 GPT 插件已安装，但未找到可调用接口，请在设置中切换为 Zotero GPT 配置模式",
    );
  }
  ...
  if (!awesomeResult) {
    throw new ReviewUserError(
      ...
      "已检测到 GPT 插件，但当前未找到可调用接口，请在设置中切换为 Zotero GPT 配置模式",
    );
  }
...
async function fetchOpenAIEmbeddings(
  ...
  const response = await fetchWithTimeout(
    endpoint,
    {
      ...
      body: JSON.stringify({
        model: ZOTERO_GPT_EMBEDDING_MODEL,
        input: inputs,
      }),
    },
