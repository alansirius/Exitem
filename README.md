...
- Auto API config selection:
  - use `zotero-gpt` config if available
  - otherwise use Exitem local config
- Compatible GPT plugin bridge mode (`zotero-gpt` / Awesome GPT compatible path)
- OpenAI-compatible API and Google Gemini support (Exitem local config mode)
